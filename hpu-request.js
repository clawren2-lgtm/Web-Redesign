...
(function () {
  ...
  /* ────────────────────────────────────────────────────────────
     SCROLL-GATED REVEAL — state
     ----------------------------------------------------------------
     Sections no longer auto-open the instant their required fields
     are filled (and autofill no longer dumps the whole form). Instead
     the next section is revealed only when the user scrolls the BOTTOM
     of the current section above the viewport's vertical centre — a
     deliberate "I'm done here, what's next" gesture. At that moment:
       • all required fields filled  → reveal the next section
       • something required missing  → flag the empty field(s) with a
         "Required to continue" indicator and DON'T advance.
     Filling the flagged field(s) then advances immediately (we don't
     make the user scroll again to be rewarded for fixing it).
     Works identically whether values were typed or autofilled.
     To ROLL BACK to the old greedy/autofill-opens-all behavior, see
     SCROLL-REVEAL-NOTES.md (restore the two "backup greedy-reveal"
     files, or flip GATE_ENABLED below to false).
     ──────────────────────────────────────────────────────────── */
  var GATE_ENABLED = true;          /* false → legacy greedy reveal (see notes) */
  var GATE_RATIO   = 0.5;           /* trigger line = 50% of viewport height */
  var revealing    = false;         /* lock while a section is animating open */
  var firedIdx     = -1;            /* section index we've already nagged at this rest */
  var errorStage   = -1;            /* section index currently showing a missing-field flag */
  ...
  /* ============================================================
     ...
     ============================================================ */
  /* ── completion checks ───────────────────────────────────────
     A field is "required" if its .form_question carries data-required="1"
     AND is currently visible (not a hidden/conditional/system field).
     NOTE: this now includes Birthdate (3 selects) in the Contact section,
     so the asterisked Birthdate is genuinely enforced. The previous build
     gated only on [data-req] elements, which skipped Birthdate. To restore
     the old set, swap requiredFields() back to querying "[data-req]".  */
  function requiredFields(stage) {
    return [].slice.call(stage.querySelectorAll('.form_question[data-required="1"]'))
      .filter(function (q) {
        if (q.classList.contains("hidden")) { return false; }
        if (q.getAttribute("aria-hidden") === "true") { return false; }
        if (q.hasAttribute("data-hidden-by-condition")) { return false; }
        if (q.hasAttribute("data-invisible")) { return false; }
        if (q.closest(".hidden")) { return false; }
        return true;
      });
  }
  /* Is a single .form_question fully answered? Handles radio groups,
     birthdate (3 selects), address (several inputs/selects) and plain text. */
  function fieldComplete(q) {
    var radios = q.querySelectorAll('input[type="radio"]');
    if (radios.length) {
      return !!q.querySelector('input[name="' + radios[0].name + '"]:checked');
    }
    var ctrls = [].slice.call(q.querySelectorAll(
      'input[type="text"],input[type="email"],input[type="tel"],textarea,select'));
    var reqd  = ctrls.filter(function (c) {
      return c.hasAttribute("required") || c.hasAttribute("data-req");
    });
    var check = reqd.length ? reqd : ctrls;
    return check.every(function (c) { return String(c.value || "").trim() !== ""; });
  }
  function stageComplete(stage) {
    return requiredFields(stage).every(fieldComplete);
  }
  function lastOpenIndex() {
    var last = 0;
    for (var i = 0; i < stages.length; i++) {
      if (stages[i].classList.contains("is-open")) { last = i; }
    }
    return last;
  }

  /* ── missing-field indicator ─────────────────────────────────── */
  function firstControl(q) {
    return q.querySelector('input:not([type="hidden"]), select, textarea');
  }
  function setFieldError(q, on, doShake) {
    if (on) {
      q.classList.add("is-missing");
      if (!q.querySelector(".rq-fieldmsg")) {
        var msg = document.createElement("div");
        msg.className = "rq-fieldmsg";
        msg.setAttribute("role", "alert");
        msg.textContent = "Required to continue";
        q.appendChild(msg);
      }
      [].slice.call(q.querySelectorAll("input, select, textarea"))
        .forEach(function (c) { c.setAttribute("aria-invalid", "true"); });
      if (doShake && allowMotion) {
        q.classList.remove("rq-shake");
        void q.offsetWidth;               /* restart the animation */
        q.classList.add("rq-shake");
        setTimeout(function () { q.classList.remove("rq-shake"); }, 480);
      }
    } else {
      q.classList.remove("is-missing", "rq-shake");
      var m = q.querySelector(".rq-fieldmsg");
      if (m) { m.remove(); }
      [].slice.call(q.querySelectorAll("input, select, textarea"))
        .forEach(function (c) { c.removeAttribute("aria-invalid"); });
    }
  }
  /* Flag every empty required field in a stage; shake + gently surface the
     first one only if it has scrolled out of sight above the viewport. */
  function markStageErrors(stage) {
    var firstMissing = null;
    requiredFields(stage).forEach(function (q) {
      var miss = !fieldComplete(q);
      setFieldError(q, miss, miss);
      if (miss && !firstMissing) { firstMissing = q; }
    });
    if (firstMissing) {
      var r = firstMissing.getBoundingClientRect();
      if (r.bottom < 60) {  /* only if it scrolled above the top edge */
        var target = window.scrollY + r.top - window.innerHeight * 0.30;
        window.scrollTo({ top: Math.max(0, target), behavior: "smooth" });
      }
    }
  }
  function refreshStageErrors(stage) {
    requiredFields(stage).forEach(function (q) {
      setFieldError(q, !fieldComplete(q), false);   /* live-clear as fields fill */
    });
  }
  function clearStageErrors(stage) {
    requiredFields(stage).forEach(function (q) { setFieldError(q, false); });
  }
  ...
  function initEntrance() {
    ...
    if (isMobile) {
      /* Mobile: hero lead animates in on load as usual. The form card is
         handed off to initMobileFormReveal — pre-set it to hidden here
         (same JS task, no repaint flash) so the ScrollTrigger owns it. */
      G.set(".rq-hero__lead", { opacity: 0 });
      G.set(card, { opacity: 0, y: 36 });
      G.fromTo(".rq-hero__lead",
        { opacity: 0, y: 18 },
        { opacity: 1, y: 0, duration: 0.72, clearProps: "transform" });
    } else {
      /* Desktop: entrance cascade — hero lead then form card together. */
  ...
  /* ============================================================
     MOBILE FORM REVEAL — scroll-triggered fade + rise for the
     form card on small screens only. initEntrance pre-sets the
     card to opacity:0 / y:36 and defers here; this owns the
     animation. Gated via gsap.matchMedia so it tears down
     cleanly if the window is ever resized past the breakpoint.
     ============================================================ */
  function initMobileFormReveal() {
    if (!G || !window.ScrollTrigger) { return; }
    G.registerPlugin(window.ScrollTrigger);

    var mm = G.matchMedia();
    mm.add("(max-width: 767px) and (prefers-reduced-motion: no-preference)", function () {
      /* Card is already at opacity:0 / y:36 (set by initEntrance before
         first paint). Fire once the user has scrolled it meaningfully
         into view — "top 75%" requires ~100 px of downward scroll on a
         typical phone before the trigger fires. */
      window.ScrollTrigger.create({
        trigger: card,
        start: "top 75%",
        once: true,
        onEnter: function () {
          var tl = G.timeline({ defaults: { duration: 0.70, ease: "power3.out" } });
          /* form card fades and rises in */
          tl.to(card, { opacity: 1, y: 0, clearProps: "transform" }, 0);
          /* hero text block lifts up simultaneously — the 50 px of space
             that padding-bottom previously held now comes from this JS lift */
          tl.to(".rq-hero__textrow", { y: -50 }, 0);
        }
      });

      return function () {
        /* Cleaned up when resizing past the breakpoint */
        G.set(card, { clearProps: "opacity,y,transform" });
      };
    });
  }
  ...
  /* Reveal section `i` with the same animation as before, lock briefly while
     it settles, then re-measure scroll triggers and re-evaluate (so a long
     continuous scroll, or a fully-autofilled form, cascades open naturally). */
  function openStage(i) {
    if (i < 0 || i >= stages.length) { return; }
    if (stages[i].classList.contains("is-open")) { return; }
    revealing = true;
    firedIdx  = -1;
    ...
    setTimeout(function () {
      revealing = false;
      /* Deliberately NO ScrollTrigger.refresh() here. Opening a stage grows
         element/content height, not the viewport. Now that completing a section
         cascades the next one open immediately, refreshing on every open re-measured
         every trigger mid-interaction and caused scroll jank. The proof track is
         pre-measured at full expanded height and the hero-FX end is viewport-based,
         so neither needs a content-change refresh. Refreshes now fire ONLY on a real
         viewport resize / breakpoint change. */
      if (stages.every(function (s) { return s.classList.contains("is-open"); })) {
        markProofEngaged();
      }
      evaluate(false);   /* cascade if the user is already scrolled past the new section */
    }, 640);
  }

  /* The gate. Called on scroll (fromScroll=true) and on every input change
     (fromScroll=false). Single source of truth for both revealing and flagging. */
  function evaluate(fromScroll) {
    if (!GATE_ENABLED || revealing) { return; }
    var idx = lastOpenIndex();

    /* (A) Recovery: a section is currently flagged. Fixing it advances
    ...
    if (errorStage === idx) {
      ...
      if (stageComplete(es)) {
        clearStageErrors(es);
        errorStage = -1;
        if (idx < stages.length - 1) { openStage(idx + 1); }
      } else {
        refreshStageErrors(es);
      }
      return;
    }

    if (idx >= stages.length - 1) { return; }   /* last section — nothing to reveal */
    var stage  = stages[idx];

    /* (B) COMPLETION OPENS IMMEDIATELY. As soon as every required field in the
       current section is filled, reveal the next one — no scroll gesture needed.
       This fires from input/change too (fromScroll=false), so finishing a section
       (or autofilling it) cascades the next one open right away. */
    if (stageComplete(stage)) {
      openStage(idx + 1);
      return;
    }

    /* Incomplete: the scroll gate still decides when to FLAG missing fields, so we
       don't nag the moment a section appears — only once its bottom passes the line. */
    var center = window.innerHeight * GATE_RATIO;
    var past   = stage.getBoundingClientRect().bottom <= center;
    if (!past) { firedIdx = -1; return; }        /* above the line again → re-arm */

    /* never nag before the user has engaged with the form */
    if (!started) { return; }
    if (fromScroll && firedIdx !== idx) {
      firedIdx   = idx;
      errorStage = idx;
      markStageErrors(stage);
    }
  }
    ...
    G.to(line, {
      ...
      onComplete: function () {
        ...
        if (isPersonalized) {
          ...
          /* fade in the action line shortly after the greeting */
          G.to(detail, { opacity: 1, duration: 0.45, ease: "power2.out", delay: 0.3 });
  ...
  /* ENTER-TO-SUBMIT GATE — pressing Enter in a single-line field triggers an
     implicit form submit (which would open the success modal). Suppress that
     until the user has actually scrolled down the page, so an early Enter while
     filling the top fields can't submit the form by accident. Textareas keep
     their normal newline behavior; once scrolled, Enter works as usual. */
  var enterArmed = false;
  ...
  if (formEl) {
    formEl.addEventListener("keydown", function (e) {
      ...
      e.preventDefault();   /* block the implicit submit until scrolled */
    });
  }

  /* Scroll / resize drive the gate. rAF-throttled; passive. */
  var gateTicking = false;
  function onScrollGate() {
    if (!enterArmed && window.scrollY > 120) { enterArmed = true; }
    if (gateTicking) { return; }
    gateTicking = true;
    requestAnimationFrame(function () { evaluate(true); gateTicking = false; });
  }
  window.addEventListener("scroll", onScrollGate, { passive: true });
  window.addEventListener("resize", function () { evaluate(false); });
  ...
  function openSuccessModal() {
    ...
    if (subEl && emailVal) {
      // US mailing address required for the sticker promise; international
      // submitters see only the fee-waiver line. Country defaults to US.
      var countryEl = document.getElementById("form_c19cc7e8_country");
      var isUS = !countryEl || countryEl.value === "US";
      var tail = isUS
        ? "Your fee-waiver code is on its way there too, your HPU stickers are headed to the address you shared, and your personal admissions counselor will reach out within one business day."
        : "Your fee-waiver code is on its way there too, and your personal admissions counselor will reach out within one business day.";
      subEl.innerHTML = "Information is headed to <strong>" + escapeHtml(emailVal) +
        "</strong>! " + tail;
    }
  ...
  /* ============================================================
     AUTOFILL DETECTION
     Chrome fires animationstart on inputs it autofills. Collapsed
     sections are clipped by height (opacity stays 1), so Chrome
     still fills their fields even while hidden — we just DON'T open
     them all here. Instead we sync internal state and let the
     scroll gate reveal each (already-complete) section in turn, so
     autofill feels identical to typing: progressive, not a dump.
     (Legacy behavior — open everything at once — is preserved in
     the "backup greedy-reveal" files; see SCROLL-REVEAL-NOTES.md.)
     ============================================================ */
  function onAutofill() {
    markStarted();
    [].slice.call(body.querySelectorAll("select")).forEach(syncSelect);
    /* mirror autofilled radio state onto the segmented-control labels */
    [].slice.call(body.querySelectorAll(".seg input:checked")).forEach(function (i) {
      var lbl = i.closest("label");
      if (lbl) { lbl.classList.add("is-sel"); }
    });
    schedulePersonalize();
    if (errorStage >= 0) { refreshStageErrors(stages[errorStage]); }
    evaluate(false);
  }
